...
<div class="container-fluid">
        <div class="row">
            <div class="col-xs-12">
                <h1 class="page-header-heading"><span class="fa fa-circle text-success page-header-heading-icon"></span>{{$admin->name}}<small>User Profile</small></h1>
                <p class="page-header-description">{{$admin->name}} <strong>{{$admin->status}}</strong>.</p>
            </div>
        </div>
    </div>
</header>
        ...
        <div class="col-xs-12 col-lg-9 col-md-7">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <span class="fa fa-clipboard"></span>&nbsp;&nbsp;{{$admin->name}} Details
                </div>
                <div class="panel-body">
                    <div class="profile-details">
                        <div class="profile-details-profile-picture">
                            {{-- <img src="http://www.stickpng.com/assets/images/585e4bcdcb11b227491c3396.png" alt="{{ $admin->name }}"> --}}
                        </div>
                        <div class="profile-details-info">
                            <h2 class="profile-details-info-name">{{$admin->name}} <small>Technician at Megatrik</small></h2>
                            <p class="profile-details-info-summary">Biodata {{$admin->name}}:</p>
                            <ul class="profile-details-info-contact-list">
                                <li class="profile-details-info-contact-list-item">
                                    <div class="form-group">
                                        ...
                                        <p class="form-control">{{$admin->biodata->gender}}</p>
                                         <label class="label label-default">Tanggal Lahir : </label>
                                        <p class="form-control">{{$admin->biodata->date_of_birth}}</p>
                                         <label class="label label-default">CP : </label>
                                        <p class="form-control">{{$admin->biodata->cp}}</p>
                                         <label class="label label-default">Tanggal Daftar : </label>
                                        <p class="form-control">{{$admin->created_at}}</p>
                ...
                <table class="table">
                    <tbody>
                        <tr>
                            ...
                            <td>{{$admin->biodata->last_education}}</td>
                        </tr>
                        <tr>
                            <th>Profesi</th>
                            <td>{{$admin->biodata->profession}}</td>
                        </tr>
                        <tr>
                            <th>Keahlian</th>
                            <td>{{$admin->biodata->skill}}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
